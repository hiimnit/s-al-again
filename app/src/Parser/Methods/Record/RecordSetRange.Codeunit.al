codeunit 69305 "Record SetRange FS" implements "Method FS"
{
    var
    ...
    var
        ...
        RecordRef: RecordRef;
        FieldRef: FieldRef;
        FromValue, ToValue : Interface "Value FS";
        FieldId: Integer;
    begin
        ...
        FromValue := ArgumentNode.Value().Evaluate(Runtime);
        if not ArgumentNode.HasNext() then begin
            FieldRef.SetRange(FromValue.GetValue());
            exit(VoidValue);
        end;

        ArgumentNode := ArgumentNode.Next();
        ToValue := ArgumentNode.Value().Evaluate(Runtime);
        FieldRef.SetRange(
            FromValue.GetValue(),
            ToValue.GetValue()
        );
    ...
    begin
        ...
        until not ArgumentNode.HasNext();
    end;
}